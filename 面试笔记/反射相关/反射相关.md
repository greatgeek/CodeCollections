...
public class RefTest {
    public static void main(String[] args) {
        try {
            Class clazz = Class.forName("com.zy.java.RefTest");// 获取反射对象类型，Class.forName()获取会执行静态代码块
            Object refTest = clazz.newInstance();// 新建实例
            Method method = clazz.getDeclaredMethod("refMethod");// 获取其 Method 对象
...
我们在调用反射时，首先会创建 Class 对象，然后获取其 Method 对象，调用 invoke 方法。获取反射方法时，有两个方法，`getMethod`和`getDeclaredMethod`。
...
1. getMethod 中 checkMemberAccess 传入的是 `Member.PUBLIC`，而 getDeclaredMethod 传入的是 `Member.DECLARED` 。这两个值有什么区别，看一下代码中的注释：
...
![image-20200806094749722](反射相关.assets/image-20200806094749722.png)
...
#### 1.3.4 privateGetDeclareMethods
...
![image-20200806100902110](反射相关.assets/image-20200806100902110.png)
...
### 3.1 Method#invoke 方法会对参数做封装和解封操作
...
![image-20200806101744521](反射相关.assets/image-20200806101744521.png)

## 参考

[大家都说 Java 反射效率低，你知道原因在哪里么](https://juejin.im/post/6844903965725818887#heading-17)